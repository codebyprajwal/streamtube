...
const options = {
  method: 'GET',
  url: `https://youtube-v31.p.rapidapi.com/search?part=snippet&q=${id}`,
  params: {
      maxResults: 100,
    },
  headers: {
...
const {data, status} = useQuery(id, getData)
  ...
  // Recording 
      ...
      document.addEventListener('keydown', function(event){ 
        if (event.key == "ArrowLeft") {
          ...
          if (position > 0) {
            position--;
          speech.text = data[position].snippet.title;
      ...
      const eventClick = () => {
        navigate(`/Video/${data[position].id.videoId}`)
      }
        ...
        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 1 }} columns={{ xs: 4, sm: 8, md: 12 }}>
            {
                  data.map((values, index) => (
                        <Grid item  xs={3} key={index}>
                              <MediaCard eventClick={eventClick} data={values} />
                        </Grid>
                  ))
            }
      </Grid>
